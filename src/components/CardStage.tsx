...
/** 前後の文はあくまで手がかりなので、極端に長いものは丸ごと落とす。
 *  現在の文だけは途中で切らない (スクロールで読める)。 */
const NEIGHBOUR_MAX = 500;
...
function contextBlocks(
  ...
  for (let i = lo; i <= hi; i++) {
    ...
    } else if (s.end - s.start <= NEIGHBOUR_MAX) {
      blocks.push({
        id: i,
        current: false,
...
export function CardStage(props: Props) {
  ...
  const card =
    step?.kind === "card"
      ? cards[step.cardIndex]
      : step?.kind === "summary"
        ? cards[step.afterCard]
        : undefined;

  useEffect(() => {
    if (display.mode === "context")
  ...
  return (
    <div
      ...
      style={{
        background: dim
          ? "color-mix(in srgb, var(--kg-paper) 86%, #000)"
          : "var(--kg-paper)",
    ...
    >
      ...
      {step?.kind !== "summary" && display.mode !== "context" && (
        ...
        >
          <span
            aria-hidden
            className="absolute left-1/2 h-4 w-px"
            style={{ top: -30, background: "var(--kg-mark)" }}
          />
          <span
            aria-hidden
            className="absolute left-1/2 h-4 w-px"
            style={{ bottom: -30, background: "var(--kg-mark)" }}
          />
      ...
      {step?.kind === "summary" && (
        <div
          ...
          style={{
            background: "color-mix(in srgb, var(--kg-paper) 96%, #000)",
          }}
          tabIndex={0}
        ...
        >
          <div
            ...
            style={{
              background: "color-mix(in srgb, var(--kg-mark) 16%, transparent)",
            }}
          >
            <div
              className="h-full"
              style={{
                width: `${(props.summaryProgress ?? 0) * 100}%`,
      ...
      {display.mode === "context" && card && (
        ...
        >
          ...
          <div
            className="pointer-events-none absolute inset-x-0 top-3 z-10 text-center text-[10px] uppercase tracking-[0.14em]"
            style={{ color: "var(--kg-muted)" }}
          >
            文脈
          </div>
          {/* 上下に半分ぶんの余白を敷いておく。これが無いと、内容が短いときに
              現在の文を画面の中央まで持ってこられない。 */}
          <div
            className="kg-context h-full overflow-y-auto px-11"
            tabIndex={0}
            data-hotkeys-off
          >
            <div className="mx-auto grid max-w-[42em] gap-7 py-[50cqh] text-[17px] leading-[1.95]">
              {(() => {
                const c = contextBlocks(
                  source,
                  sentences,
                  card,
                  card.sentenceId,
                      ...
                      b.current ? (
                        ...
                        >
                          ...
                          <mark
                            style={{
                              background:
                                "color-mix(in srgb, var(--kg-mark) 22%, transparent)",
                              color: "var(--kg-ink)",
                              fontWeight: 600,
                            }}
                          >
